    ...
    <main>
      <section className="relative overflow-hidden border-b border-white/10">
        <div
          ...
          style={{
            background:
              "radial-gradient(600px 400px at 20% 10%, rgba(56,189,248,0.18), transparent 60%), radial-gradient(500px 360px at 80% 0%, rgba(167,139,250,0.12), transparent 55%)",
          }}
        />
        <div className="relative mx-auto max-w-6xl px-4 py-40 sm:px-6 sm:py-56">
          <p className="font-mono text-sm uppercase tracking-[0.3em] text-sky-400/90">
            {site.name}
          </p>
          ...
          <div className="mt-8 max-w-2xl border-l-2 border-sky-400/70 pl-5">
          ...
          <div className="mt-10 flex flex-wrap gap-4">
            <Link
              href="/contact"
              className="rounded-full border border-sky-400/60 bg-sky-500/15 px-6 py-3 font-sans text-sm font-semibold text-sky-50 shadow-[0_0_30px_rgba(56,189,248,0.2)] transition hover:border-sky-300 hover:bg-sky-500/25"
            >
              Talk with us
            </Link>
            <Link
              href="/process"
              className="rounded-full border border-white/15 bg-white/5 px-6 py-3 font-sans text-sm font-semibold text-zinc-100 transition hover:border-sky-400/40 hover:text-sky-100"
            >
              Our services
            </Link>
          </div>
          ...
          {posts.length === 0 ? (
            ...
            posts.map((p) => (
              <Link
                key={p.slug}
                href={`/blog/${p.slug}`}
                className="rounded-2xl border border-white/10 bg-[#141414] p-5 transition hover:border-sky-400/40"
              >
